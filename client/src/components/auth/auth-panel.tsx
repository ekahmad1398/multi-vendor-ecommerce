...
function AuthShell({ eyebrow, title, note, children }: { eyebrow: string; title: string; note: string; children: React.ReactNode }) {
  return <div className="shell grid min-h-[78vh] items-center gap-8 py-10 lg:grid-cols-[.85fr_1fr] lg:py-16"><aside className="hidden rounded-[2rem] bg-slate-950 p-10 text-white lg:block"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#63f5c6] text-slate-950"><Sparkles size={22} /></span><p className="mt-10 text-sm font-bold uppercase tracking-[.18em] text-[#63f5c6]">Morrow member</p><h2 className="mt-4 max-w-sm font-serif text-5xl leading-none tracking-[-.055em]">One account.<br />Every good find.</h2><p className="mt-6 max-w-sm leading-7 text-slate-300">Use a Morrow password or a Clerk account. Both create the same secure store session.</p><div className="mt-12 flex items-center gap-3 text-sm text-slate-300"><ShieldCheck className="text-[#63f5c6]" size={19} /> Your account and checkout stay protected.</div></aside><section className="auth-card w-full max-w-md justify-self-center p-7 sm:p-9"><div className="flex items-center gap-3 text-violet-700"><span className="grid h-10 w-10 place-items-center rounded-xl bg-violet-100"><Mail size={18} /></span><p className="eyebrow">{eyebrow}</p></div><h1 className="mt-6 font-serif text-5xl tracking-[-0.05em] text-slate-950">{title}</h1><p className="mt-3 leading-7 text-slate-600">{note}</p>{children}<p className="mt-7 flex items-center justify-center gap-2 text-xs text-slate-400"><CheckCircle2 size={14} /> Secure store session</p></section></div>;
}
...
function ClerkScreen({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  const appearance = { variables: { colorPrimary: "#6956e8", colorText: "#0b1220", colorBackground: "#ffffff", borderRadius: "0.9rem" } };
...
export function SignInPanel() {
  const router = useRouter(); const { refresh } = useLocalAuth(); const { signOut: clerkSignOut } = useClerk(); const [pending, setPending] = useState(false); const [email, setEmail] = useState(""); const search = useSearchParams();
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setPending(true); const data = new FormData(event.currentTarget); try { await clerkSignOut(); const result = await login(String(data.get("email")), String(data.get("password"))); await refresh(); toast.success(result.message); router.push("/"); } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to sign in"); } finally { setPending(false); } };
