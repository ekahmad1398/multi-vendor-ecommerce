...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
      <div className="shell flex h-9 items-center justify-between gap-4 text-[11px] font-medium text-slate-500">
      ...
      <div className="border-t border-slate-100">
        <div className="shell flex min-h-[4.75rem] items-center justify-between gap-4">
          ...
          <nav aria-label="Main navigation" className="desktop-only items-center gap-7">
            {links.map(([label, href]) => <Link key={href} href={href} className={`text-sm font-semibold ${isActive(href) ? "text-violet-700" : "text-slate-600 hover:text-slate-950"}`}>{label}</Link>)}
          </nav>
